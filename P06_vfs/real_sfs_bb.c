...
static int sfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	sb->s_blocksize = /* TODO 1: Block size for this file system */;
	sb->s_blocksize_bits = /* TODO 2: log2 of block size for this file system */;
	sb->s_magic = /* TODO 3: File system type */;
	sb->s_type = &sfs; // file_system_type
	sb->s_op = &sfs_sops; // super block operations

	// Obtain an inode from VFS
	sfs_root_inode = iget_locked(sb, /* TODO 4: Inode number for the root of this file system */);
	if (!sfs_root_inode)
	{
		return -EACCES;
	}
	...
	sb->s_root = d_alloc_root(/* TODO 5A: Root's VFS inode to be attached to VFS superblock */);
#else
	sb->s_root = d_make_root(/* TODO 5B: Root's VFS inode to be attached to VFS superblock */);
#endif
	if (!sb->s_root)
	{
