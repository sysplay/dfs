...
{
	return read_from_real_sfs(info, 0 /* TODO 7A: FS Block Number */,
				 0 /* TODO 8A: FS Block Offset */, fe, sizeof(sfs_file_entry_t));
}
static int write_entry_to_real_sfs(sfs_info_t *info, int ino, sfs_file_entry_t *fe)
{
	return write_to_real_sfs(info, 0 /* TODO 7B: FS Block Number */,
				0 /* TODO 8B: FS Block Offset */, fe, sizeof(sfs_file_entry_t));
}
int sfs_get_file_entry(sfs_info_t *info, int vfs_ino, sfs_file_entry_t *fe)
{
	return read_entry_from_real_sfs(info, 0 /* TODO 9A: Pass our file entry number */, fe);
}
int sfs_update_file_entry(sfs_info_t *info, int vfs_ino, sfs_file_entry_t *fe)
{
	return write_entry_to_real_sfs(info, 0 /* TODO 9B: Pass our file entry number */, fe);
}
...
int init_browsing(sfs_info_t *info)
{
	...
	if (1 /* TODO 5: Check for validity of Simula File System */)
	{
...
int sfs_list(sfs_info_t *info, struct file *file, void *dirent, filldir_t filldir)
{
	...
	{
		if ((retval = read_entry_from_real_sfs(info, 0 /* TODO 10A: Get the ino'th file entry into fe */, &fe)) < 0)
			return retval;
		if (!fe.name[0]) continue;
...
int sfs_list(sfs_info_t *info, struct file *file, struct dir_context *ctx)
{
	...
	for (ino = 0; ino < info->sb.entry_count; ino++)
	{
		if ((retval = read_entry_from_real_sfs(info, 0 /* TODO 10B: Get the ino'th file entry into fe */, &fe)) < 0)
			return retval;
		if (!fe.name[0]) continue;
		pos++; /* Position of this file */
		if (ctx->pos == pos)
...
/* This function is called only if the file doesn't exist */
{
	...
	fe->size = 0;
	fe->timestamp = 0; /* TODO 11: timestamp */
	fe->perms = 0; /* TODO 12: Permissions passed */
...
int sfs_lookup(sfs_info_t *info, char *fn, sfs_file_entry_t *fe)
{
	...
	for (ino = 0; ino < info->sb.entry_count; ino++)
	{
		if (read_entry_from_real_sfs(info, ino, fe) < 0)
			return INV_INODE;
		if (!fe->name[0]) continue;
		if (strcmp(fe->name, fn) == 0) return 0; /* TODO 13: Return the VFS Inode Number */
	}
...
int sfs_remove(sfs_info_t *info, char *fn)
{
	...
	/* TODO 14: Initialize fe entry to zero's */
	memset(&fe, 0, 0);
